...
import { SafepayDefaultError } from "../models/errors/safepaydefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SafepayDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SafepayDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SafepayDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SafepayDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
